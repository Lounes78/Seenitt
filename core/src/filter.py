...
class QualityFilter:
    def __init__(self, margin=5, min_score=0.8, max_border_contact_ratio=0.0):
        self.margin = margin
    ...
    def check(self, mask, score):
        ...
        # 4. Border Contact Ratio Check
        contact_region = cv2.bitwise_and(mask, border_mask)
        contact_pixels = np.count_nonzero(contact_region)
        
        ratio = contact_pixels / total_pixels
        
        if ratio > self.max_border_contact_ratio:
            return False, f"Touching Border ({ratio:.1%})"
...
# class QualityFilter:
#     def __init__(self, margin=5, min_score=0.5):
#         self.margin = margin
#         self.min_score = min_score
...
#         # 3. Create a "Border Mask"
#         # This mask is 255 only at the image borders (within margin)
#         border_mask = np.zeros_like(mask)
#         cv2.rectangle(border_mask, (0, 0), (w, h), 255, thickness=self.margin)
        
#         # 4. Strict Border Contact Check
#         # Intersection is where both the object mask and border mask are 255.
#         contact_region = cv2.bitwise_and(mask, border_mask)
        
#         if np.count_nonzero(contact_region) > 0:
#             return False, "Touching Border"
