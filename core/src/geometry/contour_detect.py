...
def calculateMask(mask_shape, contours, iters=10):
  a = time()
  sum_mask = np.zeros(mask_shape, dtype=np.int32)
  tmp_mask = np.zeros(mask_shape, dtype=np.int32)
  for i, cnt in enumerate(contours):
    ...
    # for i in [3,5,7,9,11,13,15]:
      ...
      big_cnt = (cnt*i + cnt_center*(1-i)).astype(np.int32)
      tmp_mask[:] = 0 # Reset
  ...
  if sum_mask.max() > 0:
    ...
    sum_mask = sum_mask.astype(float)
